...
public class CreateAccountSystem extends JPanel {
	...
	public CreateAccountSystem(final MenuGUI f) {
		...
		layout.putConstraint(SpringLayout.HORIZONTAL_CENTER, connexion, -frame.getWidth() / 4,
		...
		layout.putConstraint(SpringLayout.HORIZONTAL_CENTER, cancel, frame.getWidth() / 4,
				SpringLayout.HORIZONTAL_CENTER, this);
	...
	private void tryCreateAccount() throws AccountAlreadyExistsException {
		...
		if (login.length() > 2) {
			if (psswdMatch == true && (pswd.equals("") == false)) { // on
																	// verifie
																	// que les
																	// mdp sont
																	// les memes
